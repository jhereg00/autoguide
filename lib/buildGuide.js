...
function buildGuide (settings, cb) {
  ...
  mkpath(settings.assetDest + '/js', function (err) {
    fs.createReadStream(__dirname + '/../dist/assets/js/autoguide.js')
      .pipe(fs.createWriteStream(settings.assetDest + '/js/autoguide.js'));
  });
}
