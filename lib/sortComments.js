...
function normalizeTitle (title) {
  return title.replace(/[A-Z]+/g,function ($0) { return " " + $0.toLowerCase(); }).trim().replace(/\s+/g,'_');
}
// function normalizeTitleFromCamel (title) {
...
function sortComments (comments, objectOrder) {
  ...
  comments.forEach(function (c) {
    ...
    for (var i = startDepth; i < (c.path ? path.length + 1 : path.length); i++) {
      if (!c.path && i === path.length - 1) {
        var titleAtDepth = normalizeTitle(c.title);
      }
      else {
      ...
      // or if the title === '' (probably a 0 length path var)
      else if ((!c.path && i === path.length - 1) || titleAtDepth === '') {
    ...
    // extend
    var obj = eval(objectPath) || {};
    for (var prop in c) {
      obj[prop] = c[prop];
    }
    //console.log(c.title + ' : ' + objectPath);
    eval(objectPath + ' = obj');
  });
  ...
  //console.log(util.inspect(sections['app'],{showHidden: false, depth: null}));
