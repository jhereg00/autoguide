      ...
      if (stats.isDirectory()) {
        ...
        fs.readdir(path, function handleSassDir (err, fileNames) {
          ...
          function checkDone () {
            ...
            if (done === toCheck) {
              ...
              return cb(null, out.sort(function (a, b) {
                if (a.title < b.title)
                  return -1;
                else
                  return 1;
        ...
        fs.readFile(path, 'utf8', function (err, data) {
          ...
          while (foundVars = data.match(varRE)) {
            if (!fileOut.vars)
              fileOut.vars = {};
            if (/^\s*?\$/.test(foundVars[2]) && fileOut.vars[foundVars[2].replace(/\s+.+$/,'')]) {
              fileOut.vars[foundVars[1]] = fileOut.vars[foundVars[2].replace(/\s+.+$/,'')];
            }
            else
              fileOut.vars[foundVars[1]] = foundVars[2].replace(/\s*\!default/i,'');
            data = data.substr(foundVars.index + foundVars[0].length);
          }
          if (fileOut.vars) {
            // we got something!
            fileOut.filePath = path;
            fileOut.title = path.match(/\/_?([\w+\-\$]+)(\.\w{2,4}){0,1}$/)[1];
