...
function parseFile (filePath, options, cb) {
  ...
  fs.readFile(filePath, 'utf8', function (err, data) {
    ...
    for (var i = 0, len = comments.length; i < len; i++) {
      ...
      var contentLength = comments[i + 1] ? data.indexOf(comments[i + 1]) - contentStart : data.length;
      var content = data.substr(contentStart, contentLength);
      // parse it all
      ...
      // if ((comments[i].path && comments[i].path.indexOf('organisms') !== -1) || comments[i].filePath.indexOf('organisms') !== -1 ||
      //     (comments[i].path && comments[i].path.indexOf('layout') !== -1) || comments[i].filePath.indexOf('layout') !== -1)
      //   comments[i].fullWidth = comments[i].fullWidth === undefined ? true : comments[i].fullWidth;
    }
